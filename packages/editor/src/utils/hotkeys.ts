...
export const useKnownHotkeys = () => {
  ...
  const undo = useMemo<KnownHotkey>(() => {
    const hotkey = 'mod+Z';
    return { hotkey, label: t('hotkey.undo', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);

  const redo = useMemo<KnownHotkey>(() => {
    const hotkey = isWindows() ? 'mod+Y' : 'mod+shift+Z';
    return { hotkey, label: t('hotkey.redo', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);
  ...
  const openHelp = useMemo<KnownHotkey>(() => {
    const hotkey = 'F1';
    return { hotkey, label: t('hotkey.openHelp', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);
  ...
  const focusToolbar = useMemo<KnownHotkey>(() => {
    const hotkey = '1';
    return { hotkey, label: t('hotkey.focusToolbar', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);

  const focusMain = useMemo<KnownHotkey>(() => {
    const hotkey = '2';
    return { hotkey, label: t('hotkey.focusMain', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);

  const focusInscription = useMemo<KnownHotkey>(() => {
    const hotkey = '3';
    return { hotkey, label: t('hotkey.focusInscription', { hotkey: hotkeyText(hotkey) }) };
  }, [t]);
