...
export const hasButtonInTable = (data: Array<ComponentData>, element: ComponentData | undefined, type: ButtonType): boolean => {
  const parentTable = getParentTable(data, element);
  return parentTable
    ? (parentTable as TableConfig).config.components.some(
        column => column.config.asActionColumn && column.config.components.some(button => button.config.type === type)
      )
    : false;
};
