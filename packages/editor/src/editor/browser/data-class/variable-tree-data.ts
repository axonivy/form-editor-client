...
export const useVariableTreeData = () => {
  const { componentForType } = useComponents();
  ...
  const fullVariablePath = (row: Row<BrowserNode>, showRootNode: boolean = true): string => {
  ...
  const rowToCreateData = (row: Row<BrowserNode>, showRootNode: boolean = true, prefix?: string): CreateComponentData | undefined => {
  ...
  const formatVariableValue = (variablePath: string, prefix?: string): string => {
    if (!prefix) {
      return `#{${variablePath}}`;
    }
