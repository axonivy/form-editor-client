...
const TypeField = ({ label, value, onChange, validationPath }: GenericFieldProps) => {
  ...
  const updateValueAndAction = (change: string) => {
    ...
    setElement(element => {
      if (isButton(element)) {
        if (change === 'EDIT') {
          element.config.action = '#{ivyFormDataTableHandler.edit(row)}'; //just placeholder, will be set from backend
          element.config.variant = 'PRIMARY';
          element.config.name = '';
          element.config.icon = 'pi pi-pencil';
        } else if (change === 'DELETE') {
          element.config.action = '#{ivyFormDataTableHandler.delete(row)}'; //just placeholder, will be set from backend
