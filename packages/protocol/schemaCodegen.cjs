...
var schemaUri =
  'https://jenkins.ivyteam.io/job/core_json-schema/job/XIVY-13133-uischema/lastSuccessfulBuild/artifact/workspace/ch.ivyteam.ivy.dialog.form.schema/target/schema/form/11.3.0/form.json';
...
if (schemaUri.startsWith('http')) {
  loadJson(schemaUri)
    .then(schema => tsGen.compile(schema, 'form'))
    .then(ts => writeSrc(ts));
} else {
